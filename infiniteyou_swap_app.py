...
def generate_image(
    ...
    if input_image is None:
        gr.Error("Source Identity Image is required.")
        return gr.update()
    if control_image is None:
        gr.Error("Target Image (Context/Pose) is required for face swapping.")
        return gr.update()
...
def generate_examples(id_image_path, control_image_path, prompt_text, seed, enable_realism, enable_anti_blur, model_version):
    default_blur_kernel_size = 9
    default_mask_expand_ratio = 0.3
    
    from PIL import Image
    id_image = Image.open(id_image_path).convert("RGB") if id_image_path else None
    control_image = Image.open(control_image_path).convert("RGB") if control_image_path else None
    ...
    return generate_image(
        ...
        default_mask_expand_ratio
    )

'''
sample_list = [
    ['./assets/examples/man.jpg', './assets/examples/man_pose.jpg', 'A sophisticated gentleman exuding confidence. He is dressed in a 1990s brown plaid jacket with a high collar, paired with a dark grey turtleneck. His trousers are tailored and charcoal in color, complemented by a sleek leather belt. The background showcases an elegant library with bookshelves, a marble fireplace, and warm lighting, creating a refined and cozy atmosphere. His relaxed posture and casual hand-in-pocket stance add to his composed and stylish demeanor', 666, False, False, 'aes_stage2'],
    ['./assets/examples/man.jpg', './assets/examples/man_pose.jpg', 'A man, portrait, cinematic', 42, True, False, 'aes_stage2'],
    ['./assets/examples/man.jpg', './assets/examples/man_pose.jpg', 'A man, portrait, cinematic', 12345, False, False, 'sim_stage1'],
    ['./assets/examples/woman.jpg', './assets/examples/woman.jpg', 'A woman, portrait, cinematic', 1621695706, False, False, 'sim_stage1'],
    ['./assets/examples/woman.jpg', './assets/examples/woman.jpg', 'A young woman holding a sign with the text "InfiniteYou", "Infinite" in black and "You" in red, pure background', 3724009365, False, False, 'aes_stage2'],
    ['./assets/examples/woman.jpg', './assets/examples/woman.jpg', 'A photo of an elegant Javanese bride in traditional attire, with long hair styled into intricate a braid made of many fresh flowers, wearing a delicate headdress made from sequins and beads. She\'s holding flowers, light smiling at the camera, against a backdrop adorned with orchid blooms. The scene captures her grace as she stands amidst soft pastel colors, adding to its dreamy atmosphere', 42, True, False, 'aes_stage2'],
...
with gr.Blocks() as demo:
    ...
    <div style="text-align: center; max-width: 900px; margin: 0 auto;">
        <h1 style="font-size: 1.5rem; font-weight: 700; display: block;">InfiniteYou-FLUX Face Swapper</h1>
        <h2 style="font-size: 1.2rem; font-weight: 300; margin-bottom: 1rem; display: block;">Official Gradio Demo for <a href="https://arxiv.org/abs/2503.16418">InfiniteYou</a> adapted for Face Swapping</h2>
    ...
    1. **Upload a Source Identity Image containing a human face.** This image provides the face identity to be swapped onto the target. For multiple faces, only the largest face will be used.
    2. **Upload a Target Image (Context/Pose) containing a human face.** This image provides the background, scene context, and the pose for the swapped face. The largest face detected here will be the target for swapping.
    3. **Enter the text prompt to describe the desired overall image.** This guides the generation, especially the background and style, while the face swap happens.
    4. **[Optional] Adjust parameters:** Set seed, resolution, guidance scales, LoRAs, and the new **Mask Parameters** (Blur Kernel Size, Expand Ratio) for fine-tuning the swap.
    5. **Click the "Generate" button.** Enjoy!
    """)
    
    with gr.Row():
        with gr.Column(scale=3):
            with gr.Row():
                ui_id_image = gr.Image(label="Source Identity Image", type="pil", scale=3, height=370, min_width=100)

                with gr.Column(scale=2, min_width=100):
                    ui_control_image = gr.Image(label="Target Image (Context/Pose)", type="pil", height=370, min_width=100)
            ...
            with gr.Accordion("Advanced", open=False):
                ...
                with gr.Row():
                    ui_width = gr.Number(label="width", value=768)
                    ui_height = gr.Number(label="height", value=768)
                ...
                with gr.Row():
                    ui_blur_kernel_size = gr.Slider(minimum=1, maximum=301, step=2, value=9, label="Mask Blur Kernel Size (Odd)")
                    ui_mask_expand_ratio = gr.Slider(minimum=0.0, maximum=1.0, step=0.05, value=0.3, label="Mask Expand Ratio")
        ...
        with gr.Column(scale=2):
            ...
            gr.Markdown(
                ...
                - **Model Version**: `aes_stage2` is default. Try `sim_stage1` for higher ID similarity.
                - **Hyperparameters**: Usually defaults are okay. Try slightly larger `infusenet_guidance_start` (e.g., 0.1) or smaller `infusenet_conditioning_scale` (e.g., 0.9) if needed.
                - **Mask Parameters**: Adjust `Mask Blur Kernel Size` (higher=softer edges) and `Mask Expand Ratio` (higher=larger area swapped) to control blending.
                - **Optional LoRAs**: `realism` and `anti-blur` can be enabled.
                - **Gender Prompt**: Use prompt terms like 'a man', 'a woman' if needed.
                """
            )
